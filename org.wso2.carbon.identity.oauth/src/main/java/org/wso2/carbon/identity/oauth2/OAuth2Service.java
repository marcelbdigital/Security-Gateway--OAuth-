...
public class OAuth2Service extends AbstractAdmin {
    ...
    public OAuth2ClientValidationResponseDTO validateClientInfo(String clientId, String callbackURI) {
      ...
      //  if (log.isDebugEnabled()) {
            log.info(" OAuthSercice: Validate Client information request for client_id : " + clientId +
      ...
      //  }
        ...
        try {
           ...
           // if (log.isDebugEnabled()) {
                log.info("OAuthSercice: Registered App found for the given Client Id : " + clientId +
          ...
          //  }
            ...
            if (appDO.getCallbackUrl().equals(callbackURI)) {
                ...
                log.info("  OAuthSercice: Provided Callback URL does not match with the provided one.");
                validationResponseDTO.setValidClient(false);
            ...
            log.info(" OAuthSercice: Error when reading the Application Information.", e);
            validationResponseDTO.setValidClient(false);
    ...
    public OAuth2AccessTokenRespDTO issueAccessToken(OAuth2AccessTokenReqDTO tokenReqDTO) {
      ...
      //  if (log.isDebugEnabled()) {
            log.info(" OAuthSercice: Access Token Request Received with the Client Id : " + tokenReqDTO.getClientId() +
     ...
     //   }
        ...
        try {
			...
			OAuth2AccessTokenRespDTO tok = tokenIssuer.issue(tokenReqDTO);
			log.info("OAuth2Service: ENVIEM EL TOKEN: " + tok.getAccessToken() );
			
			
            return tok;

        } catch (Exception e) { // in case of an error, consider it as a system error
            log.error("Error when issuing the Cloud4all access token. ", e);
            OAuth2AccessTokenRespDTO tokenRespDTO = new OAuth2AccessTokenRespDTO();
            tokenRespDTO.setError(true);
            tokenRespDTO.setErrorCode(OAuth2ErrorCodes.SERVER_ERROR);
            tokenRespDTO.setErrorMsg("Error when issuing the Cloud4all222 access token");
            return tokenRespDTO;
        }
    }
}
