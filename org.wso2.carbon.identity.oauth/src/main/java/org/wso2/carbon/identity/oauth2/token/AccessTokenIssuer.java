...
public class AccessTokenIssuer {
	...
	//funcio marcel
	public void registerCloud4allToken(String clientID) throws Exception 
	{
	
		OAuthAppDAO dao = new OAuthAppDAO();
        OAuthAppDO app = new OAuthAppDO();
		
		app.setOauthConsumerKey(clientID);
		//String ConsumerSecret = OAuthUtil.getRandomNumber();
		app.setOauthConsumerSecret("none");
		app.setOauthVersion(OAuthConstants.OAuthVersions.VERSION_2);
		dao.addOAuthApplication(app); 
		
		
		log.info("CLOUD4ALL CONSUMER KEY: " + app.getOauthConsumerKey() );
		log.info("CLOUD4ALL SECRET KEY: " + app.getOauthConsumerSecret() );
    ...
    public OAuth2AccessTokenRespDTO issue(OAuth2AccessTokenReqDTO tokenReqDTO)
		...
		log.info("El Grant Type es: " + grantType );
		
		//Mirar si el grantType el el grantType de Cloud4all.
		if(grantType.equals("client_credentials"))
		{
			...
			try {
			 ...
			 registerCloud4allToken(clientID);
			
			} catch (Exception e) {
			log.info("ERROR al registar el token");
			}
			
			//client secret per provar!!! "none"
			tokenReqDTO.setClientSecret("none");
			
			//el handler es el de "client_credential".
			...
			String callback = tokenReqDTO.getCallbackURI();
			
			//tokenRespDTO.setCallbackURI("http://localhost:8080/playground/oauth2client");
			
			//hauria de ser aixi:
			tokenRespDTO.setCallbackURI(callback);
			
			log.info("Access TokenIssuer: TOKEN ENVIAT A " + tokenRespDTO.getCallbackURI() );
			...
			log.info("Access TokenIssuer: Cloud4all Access Token issued to client. client-id=" + tokenReqDTO.getClientId() + " Access TokenIssuer: EL TOKEN ES: " + tokenRespDTO.getAccessToken());
