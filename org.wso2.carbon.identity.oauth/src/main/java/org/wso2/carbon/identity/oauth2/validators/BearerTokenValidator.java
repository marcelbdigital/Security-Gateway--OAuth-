...
public class BearerTokenValidator implements OAuth2TokenValidator {
    ...
    public OAuth2TokenValidationResponseDTO validate( OAuth2TokenValidationRequestDTO validationReqDTO)
        ...
        log.info("Started processing token validation request of type : " + TOKEN_TYPE);
		...
		log.info("Bearertokenvalidator: EL TOKEN ES: " + accessToken);
		log.info("Bearertokenvalidator: PREPROCESSEDACCESSTOKEN ES: " + preprocessedAccessToken);
       ...
       /* if (OAuthServerConfiguration.getInstance().isCacheEnabled()) 
        ...
        long timestampSkew = OAuthServerConfiguration.getInstance().getDefaultTimeStampSkewInSeconds() * 1000;
        long currentTimeInMillis = System.currentTimeMillis();

        if ((currentTimeInMillis - timestampSkew) > (issuedTimeInMillis + validityPeriodInMillis)) {
            log.info("Access Token is expired.");
            //if (log.isDebugEnabled()) {
                log.info("Access Token : " + accessToken + " is expired." +
        ...
        if (OAuthServerConfiguration.getInstance().isCacheEnabled() && !cacheHit) {
			...
			//esborrem el accesstoken perque ja l'hem fet servir
			log.info("ESBORREM TOKEN!!");
			tokenMgtDAO.cleanUpAccessToken(preprocessedAccessToken);
